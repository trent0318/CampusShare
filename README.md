...
一个基于 Spring Boot + Vue 3 的校园资源共享与预约系统（前后端分离）。学生可以浏览并预约学校的物品（相机、球拍等）和场地（自习室、会议室等），使用后签到、完成、评价；管理员负责资源审核、上架下架、用户管理与数据统计。
...
| 技术 | 用途 |
| --- | --- |
| Vue 3（Composition API） | 页面框架 |
| Vite 5 | 构建 / 开发服务器 |
| Vue Router 4 | 前端路由 + 登录守卫 |
| Axios | HTTP 请求 + 拦截器（token 注入、统一错误处理） |
| JavaScript | 语言（未使用 TypeScript） |

> 只进行了很简单的前端搭建，主要目的还是为了测试后端的功能是否完整

---

## 系统架构

前后端分离：Vue 3 前端（`frontend/`，端口 5173）通过 HTTP 调用 Spring Boot 后端（端口 8080），开发环境由 Vite 代理 `/api` 到 8080，无需后端开启 CORS。

```
Vue 3 前端（5173）
   │  HTTP / JSON（Axios，/api 代理）
   ▼
...
> 前端为**最小可用版本**，已覆盖核心链路：登录/注册、首页、资源浏览与预约、我的预约、管理员资源 CRUD。评价、用户管理、数据统计、操作日志、分类管理等管理功能暂未提供前端 UI（后端接口已就绪）。
...
后端 API 启动在 `http://localhost:8080`。
...
启动后访问 `http://localhost:5173`。开发服务器已配置 `/api` 代理到 `http://localhost:8080`，无需后端开启 CORS。
...
│   └── src/
│       ├── api/            # 统一接口层（axios 封装 + 各模块）
│       ├── router/         # 路由 + 登录守卫
│       ├── utils/          # 响应式登录态 / 常量 / 提示
│       ├── components/     # 导航栏
│       └── views/          # 6 个页面（登录/注册/首页/资源列表/资源详情/我的预约）
├── docs/                   # 项目文档
